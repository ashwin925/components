...
export default function IceRevealCard() {
  ...
  const [iceSize, setIceSize] = useState({ width: 150, height: 200 });
  ...
  return (
    <div className={styles.wrapper}>
      <div ref={containerRef} className={styles.container} onMouseMove={handleMouseMove}>
        <div className={`${styles.card} ${stage >= 4 ? styles.revealed : ""}`}>
          <div
            ...
            style={{ "--x": mousePosition.x, "--y": mousePosition.y } as CSSProperties}
          />
          <div className={styles.cardContent}>
            <h3 className={styles.title}>Hidden Content</h3>
            <p className={styles.description}>This content was frozen in ice, but now it's breaking free!</p>
          </div>
        </div>
        ...
        {stage < 4 && (
          <div className={`${styles.iceOverlay} ${isAnimating ? styles.shaking : ""}`}>
            <img
              src={getIceImage() || "/placeholder.svg"}
              alt="Ice overlay"
              className={styles.halfIce}
              style={{ width: `${iceSize.width}px`, height: `${iceSize.height}px` }}
            />
          </div>
        )}
      </div>
      ...
      <div className={styles.controls}>
        <label>
          Width: <input type="number" value={iceSize.width} onChange={(e) => setIceSize({ ...iceSize, width: Number(e.target.value) })} />
        </label>
        <label>
          Height: <input type="number" value={iceSize.height} onChange={(e) => setIceSize({ ...iceSize, height: Number(e.target.value) })} />
        </label>
      </div>
    </div>
  );
